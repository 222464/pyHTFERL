...
class HTFERL_Layer(object):
    ...
    def __init__(self,
        ...
        self.visibleStates = np.zeroes(inputSize)
        self.visibleBiases = np.random.rand(inputSize) * (maxInitWeight - minInitWeight) + minInitWeight
        self.visibleReconstruction = np.zeroes(inputSize)

        self.hiddenSums = np.zeroes(layerSize)
        self.hiddenActivations = np.zeroes(layerSize)
        self.hiddenFeedForwardStates = np.zeroes(layerSize)
        self.hiddenFeedBackStates = np.zeroes(layerSize)
        self.hiddenBiases = np.random.rand(layerSize) * (maxInitWeight - minInitWeight) + minInitWeight
        self.hiddenDutyCycles = np.full(layerSize, sparsity)
        self.hiddenErrors = np.zeroes(layerSize)
    ...
    def activateForward(self):
        for x in range(0, self.layerSize[0]):
            for y in range(0, self.layerSize[1]):
                ...
                vCenter = (x / (self.layerSize[0] - 1) * (self.inputSize[0] - 1), y / (self.layerSize[1] - 1) * (self.inputSize[1] - 1))

                # Center of next layer field
                nCenter = (x / (self.layerSize[0] - 1) * (nextLayerHidden.size[0] - 1), y / (self.layerSize[1] - 1) * (nextLayerHidden.size[1] - 1))
                ...
                self.hiddenSums[x][y] = np.dot(self.feedForwardWeights[index], subFeedForward) + np.dot(self.lateralWeights[index], subFeedLateral) + self.hiddenBiases
    ...
    def activateFeedBack(self, nextLayerHidden):
        ...
        for x in range(0, self.inputSize[0]):
            for y in range(0, self.inputSize[1]):
                ...
                hCenter = (x / (self.inputSize[0] - 1) * (self.layerSize[0] - 1), y / (self.inputSize[1] - 1) * (self.layerSize[1] - 1))
                ...
                self.visibleReconstruction[x][y] = np.dot(self.feedForwardWeights[index].T, subHidden) + self.visibleBiases
